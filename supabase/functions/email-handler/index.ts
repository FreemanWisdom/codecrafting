...
function parseSubmission(body: SubmissionBody) {
  ...
  if (type === "waitlist" && name.length < 2) {
    return null;
  }
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (type === "waitlist") {
    const confirmationResponse = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        from: "CodingGroups <onboarding@resend.dev>",
        to: email,
        template: {
    ...
    if (!confirmationResponse.ok) {
      ...
      return jsonResponse(
        { error: "Confirmation email failed", details },
        500,
      );
    }
    ...
    const adminResponse = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        from: "CodingGroups Waitlist <onboarding@resend.dev>",
        to: ADMIN_EMAIL,
    ...
    if (!adminResponse.ok) {
      const details = await adminResponse.text();
      return jsonResponse(
        { error: "Admin notification failed", details },
        500,
      );
    }
    ...
    return jsonResponse(
      {
        message: "Success",
        details: "Waitlist confirmation and admin notification sent.",
      },
      200,
    );
  }
  ...
  if (!resendResponse.ok) {
    return jsonResponse({ error: "Resend error", details: data }, 500);
  }
